...
$commenti_utente = Propel::getTable('fdComment')->getQueryForListByModelAndFieldAndValue('Message', 'id', '1')->execute();
$t->is(count($commenti_utente), 12, 'getQueryForListByModelAndFieldAndValue ok');

$commenti_utente = Propel::getTable('fdComment')->getQueryForCommentsByUserId(3)->execute();
$t->is(count($commenti_utente), 6, 'getQueryForCommentsByUserId ok');

$commenti_utente = Propel::getTable('fdComment')->getQueryForCommentsByUserIdAndModel(3, 'Message')->execute();
$t->is(count($commenti_utente), 4, 'getQueryForCommentsByUserIdAndModel ok');

$commenti_utente = Propel::getTable('fdComment')->getQueryForCommentsByUserIdAndModelAndModelFieldValue(3, 'Message', 1)->execute();
$t->is(count($commenti_utente), 2, 'getQueryForCommentsByUserIdAndModelAndModelFieldValue ok');

$commenti_utente = Propel::getTable('fdComment')->getQueryForUsersByModelAndModelFieldValue('Message', 1)->execute();
...
$user = Propel::getTable('fdComment')->getUserForComment(1);
$t->ok($user instanceof $config['class_name'], 'getUserForComment ok');

$user = Propel::getTable('fdComment')->getUserForComment(123456789);
