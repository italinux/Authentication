...
$conn1 = $manager->getConnection('propel1');
$conn2 = $manager->getConnection('propel2');
$conn3 = $manager->getConnection('propel3');
...
// Set globally by ProjectConfiguration::configurePropel()
$t->is($manager->getAttribute(Propel::ATTR_VALIDATE), true);

// We disable validation for the propel2 connection in ProjectConfiguration::configurePropelConnectionPropel2()
$t->is($conn2->getAttribute(Propel::ATTR_VALIDATE), false);

// We set export attribute on the connection in databases.yml
$t->is($conn3->getAttribute(Propel::ATTR_EXPORT), Propel::EXPORT_TABLES);

$article = new ReflectionClass('Article');
$parent = new ReflectionClass('myPropelRecord');
