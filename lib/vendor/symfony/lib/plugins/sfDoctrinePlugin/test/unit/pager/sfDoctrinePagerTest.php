...
$query = Propel_Core::getTable('Article')->createQuery();
$query->setHydrationMode(Propel_Core::HYDRATE_NONE);
...
$pager = new sfPropelPager('Article', 10);
$pager->init();
$count = Propel_Core::getTable('Article')->createQuery()->count();
...
$pager->init();
$t->is($pager->getNbResults(), count(Propel::getTable('Article')->findByIsOnHomepage('1')), '->setTableMethod() update the query');

// Serialization test for defect #7987
$t->diag('Serialization');
$pager = unserialize(serialize($pager));
$pager->init();
$t->is($pager->getNbResults(), count(Propel::getTable('Article')->findByIsOnHomepage('1')), 'serialization preserves TableMethod functionality');
