 ...
 * @version    SVN: $Id: sfPropelDqlTask.class.php 24625 2010-12-01 00:05:40Z Kris.Wallsmith $
 */
class sfPropelDqlTask extends sfPropelBaseTask
{
  /**
   * @see sfTask
   */
...
The [propel:dql|INFO] task executes a DQL query and displays the formatted
results:

  [./symfony propel:dql "FROM User"|INFO]

You can show the SQL that would be executed by using the [--show-sql|COMMENT] option:

  [./symfony propel:dql --show-sql "FROM User"|INFO]

Provide query parameters as additional arguments:

  [./symfony propel:dql "FROM User WHERE email LIKE ?" "%symfony-project.com"|INFO]
EOF;
  }
  ...
  protected function execute($arguments = array(), $options = array())
  {
    ...
    $this->logSection('propel', 'executing dql query');
    ...
    if ($count)
    {
      ...
      {
        $results = $q->execute($arguments['parameter'], Propel_Core::HYDRATE_SCALAR);
    ...
    {
      $this->logSection('propel', 'no results found');
    }
  }
