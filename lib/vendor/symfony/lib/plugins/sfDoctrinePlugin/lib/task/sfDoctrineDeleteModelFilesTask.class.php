...
/**
 * Delete all generated files associated with a Propel model. Forms, filters, etc.
 *
 * @package    symfony
 * @subpackage propel
 * @author     Jonathan H. Wage <jonwage@gmail.com>
 * @version    SVN: $Id: sfPropelDeleteModelFilesTask.class.php 29677 2010-05-30 14:19:33Z Kris.Wallsmith $
 */
class sfPropelDeleteModelFilesTask extends sfPropelBaseTask
{
...
The [propel:delete-model-files|INFO] task deletes all files associated with certain
models:

  [./symfony propel:delete-model-files Article Author|INFO]
EOF;
  }
  ...
  protected function execute($arguments = array(), $options = array())
  {
    $paths = array_merge(
      array(
        sfConfig::get('sf_lib_dir').'/model/propel',
        sfConfig::get('sf_lib_dir').'/form/propel',
        sfConfig::get('sf_lib_dir').'/filter/propel',
      ),
      $this->configuration->getPluginSubPaths('/lib/model/propel'),
      $this->configuration->getPluginSubPaths('/lib/form/propel'),
      $this->configuration->getPluginSubPaths('/lib/filter/propel')
    );
    ...
    foreach ($arguments['name'] as $modelName)
    {
      ...
      if ($files)
      {
        ...
        {
          $this->logSection('propel', 'Aborting delete of "'.$modelName.'" files');
          continue;
        }

        $this->logSection('propel', 'Deleting "'.$modelName.'" files');
      ...
      {
        $this->logSection('propel', 'No files found for the model named "'.$modelName.'"');
      }
    }

    $this->logSection('propel', 'Deleted a total of '.$total.' file(s)');
  }
