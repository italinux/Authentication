 ...
 * @version    SVN: $Id: sfPropelBuildModelTask.class.php 24745 2010-12-02 02:14:05Z Kris.Wallsmith $
 */
...
The task read the schema information in [config/propel/*.yml|COMMENT]
from the project and all enabled plugins.

The model classes files are created in [lib/model/propel|COMMENT].

This task never overrides custom classes in [lib/model/propel|COMMENT].
It only replaces files in [lib/model/propel/base|COMMENT].
EOF;
  }
  ...
  {
    $this->logSection('propel', 'generating model classes');

    $config = $this->getCliConfig();
    $builderOptions = $this->configuration->getPluginConfiguration('sfPropelPlugin')->getModelBuilderOptions();
    ...
    foreach (sfYaml::load($schema) as $model => $definition)
    {
      ...
      if (preg_match_all('/@property (\w+) \$(\w+)/', $code, $matches, PREG_SET_ORDER))
      {
        ...
        foreach ($properties as $name => $type)
        {
          ...
          $collection = 'Propel_Collection' == $type;
