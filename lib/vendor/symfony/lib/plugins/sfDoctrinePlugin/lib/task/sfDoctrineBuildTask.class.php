 ...
 * @version    SVN: $Id: sfPropelBuildTask.class.php 23156 2010-10-17 13:08:16Z Kris.Wallsmith $
 */
...
EOF;
  ...
  protected function execute($arguments = array(), $options = array())
  {
    ...
    {
      $task = new sfPropelDropDbTask($this->dispatcher, $this->formatter);
      ...
      $task = new sfPropelBuildDbTask($this->dispatcher, $this->formatter);
      $task->setCommandApplication($this->commandApplication);
      $task->setConfiguration($this->configuration);
      $ret = $task->run();
    ...
    {
      $task = new sfPropelMigrateTask($this->dispatcher, $this->formatter);
      $task->setCommandApplication($this->commandApplication);
      $task->setConfiguration($this->configuration);
      $ret = $task->run();
