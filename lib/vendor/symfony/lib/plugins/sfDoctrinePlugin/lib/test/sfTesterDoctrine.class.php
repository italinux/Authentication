 ...
 * @version    SVN: $Id: sfTesterPropel.class.php 23810 2010-11-12 11:07:44Z Kris.Wallsmith $
 */
class sfTesterPropel extends sfTester
{
   ...
   * @param array|Propel_Query $query A Propel_Query object or an array of conditions
  ...
  public function check($model, $query, $value = true)
  {
    if (null === $query)
    {
      $query = Propel_Core::getTable($model)
        ->createQuery('a');
    }
    ...
    {
      $conditions = $query;
      $query = $query = Propel_Core::getTable($model)
        ->createQuery('a');
      foreach ($conditions as $column => $condition)
      {
        $column = Propel_Core::getTable($model)->getFieldName($column);
  ...
  public function debug($limit = null)
  {
    ...
    foreach ($events as $event)
    {
      ...
      )
      {
        ...
        echo $event->getQuery()."\n";
        echo '  Parameters: '.sfYaml::dump(sfPropelConnectionProfiler::fixParams($event->getParams()), 0)."\n";
