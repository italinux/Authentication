 ...
 * @version    SVN: $Id: sfPropelFormFilterGenerator.class.php 27842 2010-02-10 19:42:03Z Kris.Wallsmith $
 */
class sfPropelFormFilterGenerator extends sfPropelFormGenerator
{
  ...
  public function generate($params = array())
  {
    ...
    $file = sfConfig::get('sf_lib_dir').'/filter/propel/BaseFormFilterPropel.class.php';
    if (!file_exists($file))
    {
    ...
    foreach ($models as $model)
    {
      ...
      $baseDir = sfConfig::get('sf_lib_dir') . '/filter/propel';
      ...
      file_put_contents($baseDir.'/base/Base'.$model.'FormFilter.class.php', $this->evalTemplate(null === $this->getParentModel() ? 'sfPropelFormFilterGeneratedTemplate.php' : 'sfPropelFormFilterGeneratedInheritanceTemplate.php'));

      if ($isPluginModel)
      {
        $pluginBaseDir = $pluginPaths[$pluginName].'/lib/filter/propel';
        if (!file_exists($classFile = $pluginBaseDir.'/Plugin'.$model.'FormFilter.class.php'))
        {
            ...
            file_put_contents($classFile, $this->evalTemplate('sfPropelFormFilterPluginTemplate.php'));
        }
      }
      if (!file_exists($classFile = $baseDir.'/'.$model.'FormFilter.class.php'))
      {
        if ($isPluginModel)
        {
           file_put_contents($classFile, $this->evalTemplate('sfPropelPluginFormFilterTemplate.php'));
        } else {
  ...
  {
    return null === ($model = $this->getParentModel()) ? 'BaseFormFilterPropel' : sprintf('%sFormFilter', $model);
  }
}
