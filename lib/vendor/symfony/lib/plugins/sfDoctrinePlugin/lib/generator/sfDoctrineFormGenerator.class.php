...
class sfPropelFormGenerator extends sfGenerator
{
  ...
  public function generate($params = array())
  {
    ...
    $file = sfConfig::get('sf_lib_dir').'/form/propel/BaseFormPropel.class.php';
    if (!file_exists($file))
    {
    ...
    {
      $this->table = Propel_Core::getTable($model);
      $this->modelName = $model;

      $baseDir = sfConfig::get('sf_lib_dir') . '/form/propel';
      ...
      file_put_contents($baseDir.'/base/Base'.$model.'Form.class.php', $this->evalTemplate(null === $this->getParentModel() ? 'sfPropelFormGeneratedTemplate.php' : 'sfPropelFormGeneratedInheritanceTemplate.php'));

      if ($isPluginModel)
      {
        $pluginBaseDir = $pluginPaths[$pluginName].'/lib/form/propel';
        if (!file_exists($classFile = $pluginBaseDir.'/Plugin'.$model.'Form.class.php'))
        {
            ...
            file_put_contents($classFile, $this->evalTemplate('sfPropelFormPluginTemplate.php'));
        }
      }
      if (!file_exists($classFile = $baseDir.'/'.$model.'Form.class.php'))
      {
        if ($isPluginModel)
        {
           file_put_contents($classFile, $this->evalTemplate('sfPropelPluginFormTemplate.php'));
        } else {
  ...
  public function getPluginModels()
  {
    if (!$this->pluginModels)
    {
      ...
      foreach ($pluginPaths as $pluginName => $path)
      {
        ...
        foreach (sfFinder::type('file')->name('*.php')->in($path.'/lib/model/propel') as $path)
        {
          $info = pathinfo($path);
          ...
          {
            $parent = new ReflectionClass('Propel_Record');
            $reflection = new ReflectionClass($modelName);
            if ($reflection->isSubClassOf($parent))
            {
              $this->pluginModels[$modelName] = $pluginName;
              $generators = Propel_Core::getTable($modelName)->getGenerators();
  ...
  public function getManyToManyRelations()
  {
    ...
    {
      if (
        Propel_Relation::MANY == $relation->getType()
        &&
        isset($relation['refTable'])
        &&
        (null === $this->getParentModel() || !Propel_Core::getTable($this->getParentModel())->hasRelation($relation->getAlias()))
      )
      {
        $relations[] = $relation;
      }
  ...
  public function getForeignKeyNames()
  {
    ...
    foreach ($this->table->getRelations() as $relation)
    {
      if ($relation->getType() === Propel_Relation::ONE)
      {
   ...
   *
   * @param  sfPropelColumn $column
   * @return string    The name of a subclass of sfWidgetForm
   */
  public function getWidgetClassForColumn($column)
  {
    switch ($column->getPropelType())
    {
      case 'string':
  ...
  public function getWidgetOptionsForColumn($column)
  {
    ...
    else if ('enum' == $column->getPropelType() && is_subclass_of($this->getWidgetClassForColumn($column), 'sfWidgetFormChoiceBase'))
    {
  ...
  public function getValidatorClassForColumn($column)
  {
    ...
    if ($column->isForeignKey())
    {
      $validatorSubclass = 'PropelChoice';
    }
  ...
  /**
   * Get array of sfPropelColumn objects that exist on the current model but not its parent.
   *
  ...
  public function getColumns()
  {
    ...
    $parentColumns = $parentModel ? array_keys(Propel_Core::getTable($parentModel)->getColumns()) : array();

    $columns = array();
    foreach (array_diff(array_keys($this->table->getColumns()), $parentColumns) as $name)
    {
      $columns[] = new sfPropelColumn($name, $this->table);
    }
  ...
  {
    Propel_Core::loadModels($this->generatorManager->getConfiguration()->getModelDirs());
    $models = Propel_Core::getLoadedModels();
    $models =  Propel_Core::initializeModels($models);
    $models = Propel_Core::filterInvalidModels($models);
  ...
  public function getParentModel()
  {
    ...
    $builderOptions = sfConfig::get('propel_model_builder_options', array());
  ...
  {
    return null === ($model = $this->getParentModel()) ? 'BaseFormPropel' : sprintf('%sForm', $model);
  }
}
