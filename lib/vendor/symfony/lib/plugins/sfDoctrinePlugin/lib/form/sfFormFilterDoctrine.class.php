 ...
 * @version    SVN: $Id: sfFormFilterPropel.class.php 28992 2010-04-06 11:30:44Z Kris.Wallsmith $
 */
abstract class sfFormFilterPropel extends sfFormFilter
{
  ...
  /**
   * Returns a Propel Query based on the current values form the form.
   *
   * @return Query A Propel Query object
   */
  ...
  /**
   * Builds a Propel Query based on the passed values.
   *
  ...
  /**
   * Builds a Propel query with processed values.
   *
  ...
  protected function doBuildQuery(array $values)
  {
    ...
    if ($method = $this->getTableMethod())
    {
      ...
      if ($tmp instanceof Propel_Query)
      {
        $query = $tmp;
      }
    }
  ...
  protected function addForeignKeyQuery(Propel_Query $query, $field, $value)
  {
  ...
  protected function addEnumQuery(Propel_Query $query, $field, $value)
  {
    $fieldName = $this->getFieldName($field);
  ...
  protected function addTextQuery(Propel_Query $query, $field, $values)
  {
    $fieldName = $this->getFieldName($field);
  ...
  protected function addNumberQuery(Propel_Query $query, $field, $values)
  {
    $fieldName = $this->getFieldName($field);
  ...
  protected function addBooleanQuery(Propel_Query $query, $field, $value)
  {
    $fieldName = $this->getFieldName($field);
    $query->addWhere(sprintf('%s.%s = ?', $query->getRootAlias(), $fieldName), $value);
  }

  protected function addDateQuery(Propel_Query $query, $field, $values)
  {
    $fieldName = $this->getFieldName($field);
  ...
  {
    $table = Propel_Core::getTable($this->getModelName());
  ...
  {
    return Propel_Core::getTable($this->getModelName());
  }
}
