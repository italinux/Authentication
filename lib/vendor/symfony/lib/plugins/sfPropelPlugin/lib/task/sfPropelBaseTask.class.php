...
abstract class sfPropelBaseTask extends sfBaseTask
{
  ...
  {
    $finder = sfFinder::type('file')->name('*schema.xml')->prune('propel');
  ...
  {
    $finder = sfFinder::type('file')->name('*schema.yml')->prune('propel');
  ...
  protected function copyXmlSchemaFromPlugins($prefix = '')
  {
    ...
    $schemas = sfFinder::type('file')->name('*schema.xml')->prune('propel')->in($dirs);
    foreach ($schemas as $schema)
    {
      // reset local prefix
      $localprefix = '';
